...
class BayesianLastLayer(nn.Module):
    def __init__(
        ...
        Sigma_eps_data = torch.full(
            (pred_state_length,), fill_value=sigma_eps_init, device='cuda'
        )
        if fixed_sigma:
        ...
        alpha_data = torch.tensor(data=alpha_init, device='cuda')
        if fixed_alpha:
        ...
        self.Sigma_nu = self.Sigma_nu.to('cuda')

        # w_0|0 is (K, N, D, 1) with K*N*D learnable parameters.
        glorot = sqrt(6 / (phi_dim + 1))
        w_0_0 = glorot * (
            torch.rand(
                (num_GMM_components, pred_state_length, phi_dim, 1), device='cuda'
            )
            * 2
            - 1
        )
        self.w_0_0 = nn.Parameter(data=w_0_0)
        # self.w_0_0 = self.w_0_0.to('cuda')
        ...
        # self.S_chol_0_0 = self.S_chol_0_0.to('cuda')
    ...
    def _get_S_0_0(self):
        ...
        return self.S_chol_0_0 @ self.S_chol_0_0.mT + self._S_init_diag_add * torch.eye(
            self.phi_dim, device=self.S_chol_0_0.device
        )
    ...
    def reset_to_prior(self):
        ...
        self.S_tp1_t: torch.Tensor = (
            self.S_t_t + self.Sigma_nu.to(self.S_t_t.device)
        )  # A = I, [S, B, K, N, D, D]
    ...
    def get_prior(
        ...
        # A = I
        S_1_0 = S_0_0 + self.Sigma_nu.to(S_0_0.device)
    ...
    def incorporate_transition(
        ...
        best_mode_idx = e_tp1.mean(3, keepdim=True).argmin(2, keepdim=True)
        best_update_mask = torch.zeros_like(pis_tp1)
        best_update_mask = best_update_mask.scatter_(2, best_mode_idx, True)
